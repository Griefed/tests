...
$ImprovedFabricLauncherAvailable = [int][System.Net.WebRequest]::Create($ImprovedFabricLauncherUrl).GetResponse().StatusCode
...
Function RunJavaCommand
{
    param ($CommandToRun)

    Write-Host "$script:Java $CommandToRun"
    Invoke-Expression "$script:Java $CommandToRun"
}
...
Function SetupForge
{
    ...
    if ((DownloadIfNotExists $script:ForgeJarLocation "forge-installer.jar" $script:ForgeInstallerUrl))
    {
...
Function SetupQuilt
{
    ...
    if ((DownloadIfNotExists "quilt-server-launch.jar" "quilt-installer.jar" $script:QuiltInstallerUrl))
    {
        "Installer downloaded. Installing..."
        RunJavaCommand "-jar quilt-installer.jar install server $script:MinecraftVersion --download-server --install-dir = ."
...
{
    ""
    if (($script:ModLoader -eq "Fabric") -and ($script:ImprovedFabricLauncherAvailable -eq "200"))
    {
        "Skipping Minecraft Server JAR checks because we are using the improved Fabric Server Launcher."
    }
    else
    {
        (DownloadIfNotExists $script:MinecraftServerJarLocation $script:MinecraftServerJarLocation $script:MinecraftServerUrl) > $null
    }
    ""
}
